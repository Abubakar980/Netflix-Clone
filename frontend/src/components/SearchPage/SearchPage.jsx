...
//         `http://localhost:8000/api/v1/search/${activeTab}/${searchTerm}`,
//         {
...
const SearchPage = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8000/api/v1/search/${activeTab}/${encodeURIComponent(searchTerm)}`,
        {
          withCredentials: true,
        }
      );

      // Backend sends results inside "content"
      setResults(res.data.content || []);
    } catch (error) {
      if (error.response?.status === 404) {
        toast.error(
  ...
  return (
    <div className="bg-black min-h-screen text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {results.map((result) => {
            // For movie or tv: check poster_path, for person: check profile_path
            if (
              !result.poster_path &&
              !result.profile_path
            )
